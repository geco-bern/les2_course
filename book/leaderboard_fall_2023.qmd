...
Training data is generated through a random subset of 200 pixels for each class in the @fritz_global_2017 land-use and land-cover validation dataset, restricted to locations with >80% coverage (per land cover type), located on the northern hemisphere (following the chapter's example). Locations for the training data are provided in a separate file. The equivalent test data for this competition consists of 100 unlabelled locations providing the same spectral information as provided in the training data.
...
Leaderboard submissions will be accepted two (2) times during the semester, with a final submission included with your overall report. The CSV file submitted **via pull request** should be named `xyz_results.csv` (with xyz replaced by your github handle). Submissions via email will not be considered.
